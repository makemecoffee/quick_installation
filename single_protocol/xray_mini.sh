...
_validate_port() {
    ...
    # 检查端口是否已被占用
    if jq -e ".inbounds[] | select(.port == ${port})" "$CONFIG_FILE" >/dev/null 2>&1; then
...
_install_dependencies() {
    ...
    if [ "$OS_TYPE" = "alpine" ]; then
        # 检查 Alpine 依赖
        local missing_deps=()
        for pkg in curl jq openssl wget bash xxd; do
        ...
        # 检查 Debian/Ubuntu 依赖
        local missing_deps=()
        for pkg in curl jq openssl wget xxd; do
...
_install_xray() {
    ...
    if [ "$OS_TYPE" = "alpine" ]; then
        bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/alpinelinux/install-release.sh)" || _error "Xray 安装失败"
    else
        bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install || _error "Xray 安装失败"
    fi
...
_create_global_command() {
    ...
    if [[ "$0" =~ ^/dev/fd/ ]] || [[ "$0" == "bash" ]] || [[ ! -f "$0" ]]; then
        # 通过管道执行，需要重新下载脚本
        local script_url="https://raw.githubusercontent.com/makemecoffee/quick_installation/refs/heads/master/single_protocol/xray_mini.sh"
        _info "检测到通过管道执行，正在下载脚本..."
        
        if curl -fsSL "$script_url" -o "$SCRIPT_PATH" 2>/dev/null; then
            chmod +x "$SCRIPT_PATH"
            _success "全局命令已创建！现在可以使用 'xrm' 命令"
        else
...
_main_menu() {
    while true; do
        ...
        # 显示 Xray 版本信息或未安装状态
        if [ -f "$XRAY_BIN" ]; then
            local xray_version=$(${XRAY_BIN} version 2>/dev/null | head -n1 | awk '{print $2}')
